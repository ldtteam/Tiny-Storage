...
import com.smithsmodding.tinystorage.common.inventory.implementations.ContainerTinyChest;
import com.smithsmodding.tinystorage.common.item.ItemStorageComponent;
...
public class TileEntityWoolChest extends TileEntityTinyStorage implements ISidedInventory {
	...
	public TileEntityWoolChest(int metaData) {
		super();
	...
	public String getInventoryName() {
		if (this.hasCustomName()) {
			...
			return Names.Containers.WOOL_CHEST_LOCKED;
		} else {
			return Names.Containers.WOOL_CHEST;
		}
	}
	...
	public TileEntityWoolChest applyUpgradeItem(ItemStorageComponent itemStorageComponent, int upgradeTier, EntityPlayer player) {
		...
		TileEntityWoolChest newEntity;
		if (upgradeTier == 0) {
			newEntity = new TileEntityWoolChestSmall();
		} else if (upgradeTier == 1) {
			newEntity = new TileEntityWoolChestMedium();
		} else if (upgradeTier == 2) {
			newEntity = new TileEntityWoolChestLarge();
		} else {
			return null;
		}
	...
	public TileEntityWoolChest applyDowngradeClick(World world, ItemDebugTool itemDebugTool, int upgradeTier, EntityPlayer player) {
		if (this.hasUniqueOwner() && !this.getUniqueOwner().equals(player.getGameProfile().getId().toString() + player.getDisplayName())) {
			return null;
		}
		if (numPlayersUsing > 0) {
